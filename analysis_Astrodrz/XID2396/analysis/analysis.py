...
psf_a, psf_a_std=psf_ave(psf_list,mode = 'CI', not_count=(1,2,3,4,5,6,7,8), #2,3,5,9
                  mask_list=mask_list)

psf_b, psf_b_std=psf_ave(psf_list,mode = 'CI', not_count=(0,1,2,3,4,5,9), #6,7,8
                  mask_list=mask_list)

psf_XID2138= pyfits.getdata('../../PSF_legacy/XID2138_PSF.fits')
psf_std_XID2138= pyfits.getdata('../../PSF_legacy/XID2138_PSF_std.fits')


#prf_list = [QSO_im,psf_a, psf_b, psf_XID2138]
#scal_list = [1,1,1,1]
#prf_name_list = ['QSO', 'Plan a', 'Plan b','psf_XID2138']
...
fit_result = open('fit_result_fix_x.txt','w') 
...
#fit_result.write(repr(result)+ "\n")
###############################Fit
#print "by psf XID2138"
#fixcenter = True
#source_result, ps_result, image_ps, image_host, data_C_D=fit_qso(QSO_im[cut:-cut,cut:-cut], psf_ave=psf_XID2138, psf_std = psf_std_XID2138, QSO_msk=QSO_msk,
#                                                       source_params=None, image_plot = True, corner_plot=False, flux_ratio_plot=True,
#                                                       deep_seed = False, fixcenter= fixcenter,background_rms=background_rms, pix_sz = 'drz06')
#result = transfer_to_result(data=QSO_im[cut:-cut,cut:-cut],pix_sz = 'drz06',
#                            source_result=source_result, ps_result=ps_result, image_ps=image_ps, image_host=image_host, data_C_D=data_C_D,
#                            cut=cut, filt=filt, fixcenter=fixcenter,ID=ID, savepng=True, plot_compare= True)
##fit_result.write("#fit with PSF by psf XID2138: \n")
##fit_result.write(repr(result) + "\n")
###############################Fit
#print "by psf XID2138, relax center"
#fixcenter = False
#source_result, ps_result, image_ps, image_host, data_C_D=fit_qso(QSO_im[cut:-cut,cut:-cut], psf_ave=psf_XID2138, psf_std = psf_std_XID2138, QSO_msk=QSO_msk,
#                                                       source_params=None, image_plot = True, corner_plot=False, flux_ratio_plot=True,
#                                                       deep_seed = False, fixcenter= fixcenter,background_rms=background_rms, pix_sz = 'drz06')
...
fixcenter = True
source_result, ps_result, image_ps, image_host, data_C_D=fit_qso(QSO_im[cut:-cut,cut:-cut], psf_ave=psf_b, psf_std = psf_b_std, fix_n=4.0,
                                                       source_params=None, image_plot = True, corner_plot=False, flux_ratio_plot=True,
                                                       deep_seed = False, fixcenter= fixcenter, background_rms=background_rms, pix_sz = 'drz06')
result = transfer_to_result(data=QSO_im[cut:-cut,cut:-cut],pix_sz = 'drz06',
                            source_result=source_result, ps_result=ps_result, image_ps=image_ps, image_host=image_host, data_C_D=data_C_D,
...
fixcenter = True
source_result, ps_result, image_ps, image_host, data_C_D=fit_qso(QSO_im[cut:-cut,cut:-cut], psf_ave=psf_b, psf_std = psf_b_std, fix_n=1.5,
                                                       source_params=None, image_plot = True, corner_plot=False, flux_ratio_plot=True,
                                                       deep_seed = False, fixcenter= fixcenter, background_rms=background_rms, pix_sz = 'drz06')
result = transfer_to_result(data=QSO_im[cut:-cut,cut:-cut],pix_sz = 'drz06',
                            source_result=source_result, ps_result=ps_result, image_ps=image_ps, image_host=image_host, data_C_D=data_C_D,
