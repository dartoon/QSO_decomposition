...
if if_file == []:
    ...
    fit_result.read()
count = 0
for i in np.array([1,3]):
    print "by PSF{0}".format(i)
    tag = 'fit_result_each/ps_fit_side_msk_PSF{0}'.format(i)
...
for i in np.array([1,3]):
    print "by PSF{0}".format(i)
    tag = 'fit_result_each/qso_fit_side_msk_PSF{0}'.format(i)
    mask_list = glob.glob("PSF{0}_*.reg".format(i))
    print mask_list
    psf_i = psf_list[i] * PSF_mask_img_list[i]
    psf_i = psf_i[ct:-ct,ct:-ct]
...
for i in range(len(PSF_aves)):
    ...
    tag = 'fit_result/comb_PSF_side_msk_{0}'.format(psf_com[i])
    ...
    fit_result.write("#\nfit selected PSF{0} with side_msk: \n".format(psf_com[i]))
    fit_result.write(repr(result) + "\n")
fit_result.close()
