...
msk = pyfits.getdata('{0}_msk.fits'.format(ID))
ex_name = glob.glob("QSO_msk_?.reg")
ex_msk = cr_mask_img(msk, ex_name)
plt.imshow((ex_msk * msk), origin='low') 
plt.show()
pyfits.PrimaryHDU(ex_msk * msk).writeto('{0}_msk.fits'.format(ID),overwrite=True)
