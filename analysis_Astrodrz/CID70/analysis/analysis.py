...
# Combining fitting
#==============================================================================
PSF_sub_list = np.array([ 0.00134683,  0.00044804,  0.0031444 ,  0.00355466,  0.00381107]) #np.zeros(len(psf_list)) #np.array([])
psf_list_sub = [psf_list[i]-PSF_sub_list[i] for i in range(len(psf_list))]
mask_list = glob.glob("PSF*.reg")   # Read *.reg files in a list.
psf_ave_pa, psf_std_pa=psf_ave(psf_list_sub,mode = 'CI', not_count=(0,1),
                  mask_list=mask_list)
...
#fig_pro_compare = profiles_compare(prf_list, scal_list, prf_name_list=prf_name_list,norm_pix = 3.5,
#                                   gridspace = 'log',if_annuli=True,astrodrz=True)
plt.show()
print "by selected PSF, relax center"
fixcenter = False
fit_result = open('fit_result/comb_PSF234_subsky.txt'.format(i),'w')
tag = 'fit_result/PSF_234_subsky'
source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im-0.0012442645074224009, psf_ave=psf_ave_pa, psf_std = psf_std_pa, background_rms=background_rms,
