...
# =============================================================================
#QSO_bkg_value= 0.000542728698673
#PSF_bkg_values = np.array([ 0.00051943,  0.00031653,  0.0005746 ,  0.00072594,  0.00042326,
#        0.00086457,  0.00036417,  0.00040035,  0.0005076 ])
QSO_bkg_value= 0. 
PSF_bkg_values = np.zeros(11)

psf_name_list = glob.glob("PSF?.fits") + glob.glob('not_count_*fits')  # Read *.reg files in a list.
psf_list = []
...
##==============================================================================
PSF_mask_list = glob.glob("PSF*.reg")   # Read *.reg files in a list.
psf_ave_pa, psf_std_pa=psf_ave(psf_list,mode = 'CI', not_count=(0,2,3,4,5,6),   #178
                  mask_list=PSF_mask_list)

psf_ave_pb, psf_std_pb=psf_ave(psf_list,mode = 'CI', not_count=(0,1,2,3,4,5),   #678
                  mask_list=PSF_mask_list)

psf_ave_pc, psf_std_pc=psf_ave(psf_list,mode = 'CI', not_count=(0,2,3,4,5,8),   #167
                  mask_list=PSF_mask_list)
