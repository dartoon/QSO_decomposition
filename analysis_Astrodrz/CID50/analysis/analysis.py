...
for i in range(len(psf_name_list)+2):
...
psf_ave_pa, psf_std_pa=psf_ave(psf_list,mode = 'CI', not_count=(0,2,3,4,5,6),
                  mask_list=mask_list)
#
psf_ave_pb, psf_std_pb=psf_ave(psf_list,mode = 'CI', not_count=(0,1,2,3,4,5),
                  mask_list=mask_list)
#
...
###############################Fit
PSF_sub_list = np.zeros(len(psf_list))
#
#print "each PSF, with fit_ps"    
#fit_result = open('fit_result/each_PSF_fit_ps.txt'.format(i),'w') 
#for i in np.array([1,2,3,5,6,7,8]):
#    print "by PSF{0}".format(i)
#    tag = 'fit_result/PSF{0}_ps'.format(i)
...
#    source_result, ps_result, image_ps, image_host, error_map=fit_ps(QSO_im-0., psf_ave=psf_i, psf_std = None,
...
#    fit_result.write('PSF_total_flux:{0} \n'.format(round(np.sum(psf_list[i][ct:-ct,ct:-ct]),2)))
#    ps_amp = round(ps_result[0]['point_amp'][0],2)
#    host_amp = round(image_host.sum(),2)
...
#print "each PSF, with fit_qso"    
#fixcenter = False
#fit_result = open('fit_result/each_PSF_fit_qso.txt'.format(i),'w') 
#for i in np.array([6,7,8]):
#    print "by PSF{0}".format(i)
#    tag = 'fit_result/PSF{0}_fit_qso'.format(i)
#    mask_list = glob.glob("PSF{0}*.reg".format(i))
#    print mask_list
#    psf_i, _=psf_ave([psf_list[i]-PSF_sub_list[i]],mode = 'CI', not_count=(), mask_list=mask_list)
#    psf_i = psf_i[ct:-ct,ct:-ct]
...
fixcenter = False
fit_result = open('fit_result/PSF_178_no_PSFstd.txt'.format(i),'w')
tag = 'fit_result/PSF_178_frame{0}_no_PSFstd'.format(frame)
source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im, psf_ave=psf_ave_pa, psf_std = None, background_rms=background_rms,
...
print "by selected PSF, relax center"
fixcenter = False
fit_result = open('fit_result/PSF_678_no_PSFstd.txt'.format(i),'w')
tag = 'fit_result/PSF_678_frame{0}_no_PSFstd'.format(frame)
source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im, psf_ave=psf_ave_pb, psf_std = None, background_rms=background_rms,
                                                       source_params=None, deep_seed = False, fixcenter= fixcenter, pix_sz = 'drz06',  no_MCMC =True,
                                                       QSO_msk = QSO_msk, QSO_std =QSO_std, tag=tag)
result = transfer_to_result(data=QSO_im, pix_sz = 'drz06',
                            source_result=source_result, ps_result=ps_result, image_ps=image_ps, image_host=image_host, error_map=error_map,
