...
for i in range(len(PSF_aves)):
    ...
    source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im, psf_ave=PSF_aves[i], psf_std = None, # PSF_stds[i]**2,
    ...
    fit_result.write("#fit selected PSF{0}, not PSF_std: \n".format(psf_com[i]))
    fit_result.write(repr(result) + "\n")
fit_result.close()
