...
#                                               plt_QSO = plt_QSO, norm_pix = 6, astrodrz = True,
...
psf_a, psf_a_std=psf_ave(psf_list,mode = 'CI', not_count=(1,2,6,7,8,9),
                  mask_list=mask_list)

psf_b, psf_b_std=psf_ave(psf_list,mode = 'CI', not_count= (0,1,2,3,4,5,6,10),
                  mask_list=mask_list)
...
result = transfer_to_result(data=QSO_im[cut:-cut,cut:-cut],pix_sz = 'drz06',
                            source_result=source_result, ps_result=ps_result, image_ps=image_ps, image_host=image_host, data_C_D=data_C_D,
                            cut=cut, filt=filt, fixcenter=fixcenter,ID=ID, plot_compare= True,QSO_msk = "QSO_msk*.reg")
