...
extra_psfs_IR = np.array([[2044,2122],[1836,1168],[2286,1197]])
psf_list_IR = np.vstack((psf_list_IR, extra_psfs_IR))
flux_IR = np.zeros(len(psf_list_IR))
flux_QSO_IR = 107.309   # Use the value as taking by plan b
# =============================================================================
...
extra_psfs_acs = np.array([[3137,4080],[2686,2040],[3652,2103]])
psf_list_acs = np.vstack((psf_list_acs, extra_psfs_acs))
flux_acs = np.zeros(len(psf_list_acs))
flux_QSO_acs = 102.187  # Use the value as taking by plan b
# =============================================================================
#======= if need to cal. the QSO flux from the image.=============================
