...
extra_psfs_IR = np.array([[654,1932],[708,1467],[881,1527],[1915,937]])
psf_list_IR = np.vstack((psf_list_IR, extra_psfs_IR))
flux_IR = np.zeros(len(psf_list_IR))
flux_QSO_IR = 42.58   # Use the value as taking by plan b
# =============================================================================
...
extra_psfs_acs = ([[2213,3244],[2327,2243],[2698,2380],[4910,1118]])
psf_list_acs = np.vstack((psf_list_acs, extra_psfs_acs))
flux_acs = np.zeros(len(psf_list_acs))
flux_QSO_acs = 101.478  # Use the value as taking by plan b
# =============================================================================
#======= if need to cal. the QSO flux from the image.=============================
