...
id1 = 19
id2 = 113  #54
psf_1 = psf_list[id1]/psf_list[id1].sum() * x**2
psf_2 = psf_list[id2]/psf_list[id2].sum() * x**2

psf_list = [psf_1,psf_2]
psf_i, psf_i_std = psf_ave(psf_list,mode = 'CI_tot')   #0157

for i in np.array([0]):
    print "by PSF: {0} {1}".format(id1,id2)
    tag = 'fit_test/qso_fit_PSF{0}_x={1}'.format(psf_name_list[i],x)
#    psf_i = psf_list[i] #* PSF_mask_img_list[i]
    psf_i = psf_i[ct:-ct,ct:-ct]
    psf_i_std = psf_i_std[ct:-ct, ct:-ct]
    source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im, psf_ave=psf_i, psf_std = psf_i_std**2,
                                                                     background_rms=background_rms,
    ...
    if count == 0:
        fit_result.write("#QSO_img intensity: {0} \n".format(round(np.sum(QSO_im*QSO_msk),2)))
    fit_result.write("#fit by PSF{0} {1}: \n".format(id1, id2))
