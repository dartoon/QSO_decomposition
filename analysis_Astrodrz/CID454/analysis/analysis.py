...
##############################Fit
PSF_sub_list = np.array([  0.00000000e+00,   1.34682861e-03,   0.00000000e+00,
         1.75708502e-03,   1.40350877e-04,   3.77867746e-05,
        -1.43049933e-04,   2.65587045e-03,   2.34817814e-03,
         1.40350877e-04])

#fit_result = open('fit_result/each_PSF_fit_ps.txt'.format(i),'w') 
#for i in np.array([1,3,4,5,6,7,8,9]):
...
#    source_result, ps_result, image_ps, image_host, error_map=fit_ps(QSO_im-0.0002941970310, psf_ave=psf_i, psf_std = None,
...
#    psf_i = (psf_list[i]-PSF_sub_list[i]) * cr_mask_img(image=psf_list[i], mask_list=mask_list)
#    psf_i = psf_i[ct:-ct,ct:-ct]
#    source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im-0.0002941970310, psf_ave=psf_i, psf_std = None,
#                                                                     background_rms=background_rms,
...
psf_list_sub = psf_list
for i in (7,8,9):
    psf_list_sub[i] = psf_list[i]-PSF_sub_list[i] 
mask_list = glob.glob("PSF*.reg")   # Read *.reg files in a list.
...
fixcenter = False
fit_result = open('fit_result/comb_PSF789_subsky_no_PSFstd.txt'.format(i),'w')
tag = 'fit_result/comb_PSF_789_subsky_no_PSFstd'
source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im-0.0002941970310, psf_ave=psf_ave_pa, psf_std = None, background_rms=background_rms,
                                                       source_params=None, deep_seed = False, fixcenter= fixcenter, pix_sz = 'drz06',  no_MCMC =True,
                                                       QSO_msk = QSO_msk, QSO_std =QSO_std, tag=tag)
result = transfer_to_result(data=QSO_im, pix_sz = 'drz06',
                            source_result=source_result, ps_result=ps_result, image_ps=image_ps, image_host=image_host, error_map=error_map,
                            filt=filt, fixcenter=fixcenter,ID=ID,QSO_msk = QSO_msk,tag=tag)
fit_result.write("#fit selected PSF: \n")
fit_result.write(repr(result) + "\n")
fit_result.close()