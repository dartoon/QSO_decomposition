...
test_n = 10
img = pyfits.getdata('CID1174_cutout_outer.fits')
center = np.asarray(img.shape) /2
mask_list = glob.glob("QSO_outer_msk*.reg")   # Read *.reg files in a list.
r_flux, r_grids, regions=flux_profile(img-0.00026, center,radius=center.min()/2, grids=50, ifplot=True, fits_plot= True, mask_list=mask_list)
...
r_flux, r_grids, regions=flux_profile(img-0.0001, center,radius=center.min(), grids=30, ifplot=True, fits_plot= True, mask_list=mask_list)
print r_flux[-test_n:],'\n',r_flux[-test_n:][1:]-r_flux[-test_n:][:-1], '\n', (r_flux[-test_n:][1:]-r_flux[-test_n:][:-1]).sum()
