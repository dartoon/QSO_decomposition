...
##==============================================================================
PSF_mask_list = glob.glob("PSF*.reg")   # Read *.reg files in a list.
psf_ave_pa, psf_std_pa=psf_ave(psf_list,mode = 'CI', not_count=(0,2,4,5,6,7),   #1389
                  mask_list=PSF_mask_list)

psf_ave_pb, psf_std_pb=psf_ave(psf_list,mode = 'CI', not_count=(0,4,5,7,8,9),   #1236
                  mask_list=PSF_mask_list)

#psf_ave_pc, psf_std_pc=psf_ave(psf_list,mode = 'CI', not_count=(),   #XXX
#                  mask_list=PSF_mask_list)
...
filename = 'fit_result/comb_PSF_extQSOmsk.txt'
if_file = glob.glob(filename)   
if if_file == []:
    fit_result =  open(filename,'w') 
elif if_file is not []:
...
for i in range(len(PSF_aves)):
    ...
    tag = 'fit_result/comb_PSF_{0}_extQSOmsk'.format(psf_com[i])
