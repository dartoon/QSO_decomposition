...
##############################Fit
PSF_sub_list = np.array([  5.50607287e-04,   8.58299595e-04,   3.45479082e-04,
         9.09581646e-04,   2.42914980e-04,   4.99325236e-04,
         3.77867746e-05,   1.14170040e-03,   1.80836707e-03,
         1.29554656e-03])
...
#    source_result, ps_result, image_ps, image_host, error_map=fit_ps(QSO_im-0.000653171390013, psf_ave=psf_i, psf_std = None,
...
#    psf_i = (psf_list[i]-PSF_sub_list[i]) * cr_mask_img(image=psf_list[i], mask_list=mask_list)
#    psf_i = psf_i[ct:-ct,ct:-ct]
#    source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im-0.000653171390013, psf_ave=psf_i, psf_std = None,
#                                                                     background_rms=background_rms,
...
mask_list = glob.glob("PSF*.reg")   # Read *.reg files in a list.
psf_ave_pa, psf_std_pa=psf_ave(psf_list_sub,mode = 'CI', not_count=(0,4,5,6,7,8,9),
                  mask_list=mask_list)
...
#plt.show()
#fig_pro_compare.savefig('QSO_0257.pdf')
print "by selected PSF, relax center"
fixcenter = False
fit_result = open('fit_result/comb_PSF123_sub_sky_PSFstdSmooth.txt'.format(i),'w')
tag = 'fit_result/comb_PSF123_sub_sky_PSFstdSmooth'
source_result, ps_result, image_ps, image_host, error_map=fit_qso(QSO_im-0.000653171390013, psf_ave=psf_ave_pa, psf_std = psf_std_pa, background_rms=background_rms,
