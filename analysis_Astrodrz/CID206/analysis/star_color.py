...
extra_psfs_IR = np.array([[688,1982],[1015,2100],[1879,578]])
psf_list_IR = np.vstack((psf_list_IR, extra_psfs_IR))
flux_IR = np.zeros(len(psf_list_IR))
flux_QSO_IR = 137.726   # Use the value as taking by plan b
# =============================================================================
...
acs_psf_list, QSO_loc = grab_pos(filename_1,reg_ty = 'acs', QSO_reg_return=True)
...
QSO_pos_acs = acs_psf_list[QSO_loc]
count=0
psf_list_acs = np.delete(acs_psf_list, (QSO_loc), axis=0) #copy.deepcopy(acs_psf_list[:-1])
psf_list_acs = psf_list_acs[psf_list_acs[:,0].argsort()]
extra_psfs_acs =  np.array([[351,3092],[1055,3358]])
psf_list_acs = np.vstack((psf_list_acs, extra_psfs_acs))
flux_acs = np.zeros(len(psf_list_acs))
flux_QSO_acs = 80.546  # Use the value as taking by plan b
# =============================================================================
#======= if need to cal. the QSO flux from the image.=============================
...
QSO_star_color(img_IR, QSO_pos_IR, QSO_mags, psf_list_IR, mag_IR, mag_acs_corrs,
               mag_diff, ID=ID, reg_ty='astrodrz_06',label_shift_NO=(4,5,9,),shift_where=(3,2,3,))
