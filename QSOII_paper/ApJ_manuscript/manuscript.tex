...
\submitjournal{ApJ}
\shorttitle{A Successful Observational Test}
...
\begin{document}
\title{A successful observational test of black hole and galaxy co-evolution models since $z\sim1.7$}
...
\pink{To examine the scaling relations between the mass of a supermassive black hole (SMBH) and its host galaxy properties, we adopt the recent measurements based on 32 X-ray-selected broad-line Active Galactic Nucleus (AGNs) at $1.2<z<1.7$, and} make direct comparison with two independent state-of-the-art projects, including the hydrodynamic simulation \texttt{MassiveBlackII} (\mbii) and the semi-analytic model (\sam).  After applying the observational selection function to the simulations to account for the bias, we find that both \mbii\ and \sam\ agree well with the data, in terms of the central distribution. However, accounting for observational errors, the dispersion in the mass ratio between \mbh\ and \mstar\ is significantly more consistent with the \mbii\ prediction ($\sim0.3$~dex), than with the \sam\ ($\sim0.7$~dex) one. Hence, our observations can distinguish between the different recipes adopted in the models. The mass relations in the \mbii\ are highly dependent on AGN feedback while the relations in the \sam\ are more sensitive to galaxy merger events triggering nuclear activity. Moreover, the intrinsic scatter in the mass ratio of our high-$z$ sample is comparable to that observed in the local sample, all but ruling out the proposed scenario the correlations are purely stochastic in nature arising from some sort of cosmic central limit theorem. Our results support the hypothesis of AGN feedback being responsible for a causal link between the SMBH and its host galaxy, resulting in a tight correlation between their respective masses.
...
% The work that we want to do
	...
	%Any expectation?
	%mention the scatter.
In this section, we introduce the comparison samples of the scaling relations, using the observed data (Section~\ref{hst_sample}) and the predicted ones using two independent numerical simulations (Section~\ref{sample_sim}).}
...
\blue{We employ \hst/WFC3 to obtain high-resolution (0\farcs0642 per pixel) imaging data and adopt a state-of-the-art technique to perform a 2-D flux profile decomposition to measure the light of the host galaxy.
