...
fit=scipy.optimize.curve_fit(lfit,r_band_magnitudes_selected, bhmass_selected)
fit_err=np.sqrt(np.diag(fit[1]))
r_band_space=np.linspace(-26,-18,100)
lmbh_space=lfit(r_band_space,fit[0][0],fit[0][1])
plt.plot(r_band_space,lmbh_space,color='red',linewidth=3)
lmbh_space_ub=lfit(r_band_space,fit[0][0]-fit_err[0]/2,fit[0][1]+fit_err[1]/2)
lmbh_space_lb=lfit(r_band_space,fit[0][0]+fit_err[0]/2,fit[0][1]-fit_err[1]/2)
plt.fill_between(r_band_space,lmbh_space_lb,lmbh_space_ub,color='red',alpha=0.15)

def lfit_fixm(x,c):
    return fit[0][0]*x+c
fit_fixm=scipy.optimize.curve_fit(lfit_fixm,M_r_obs, bh_mass_obs)
#fit_err=np.sqrt(np.diag(fit[1]))
lmbh_space=lfit_fixm(r_band_space,fit_fixm[0])
plt.plot(r_band_space,lmbh_space,color='green',linewidth=3)
