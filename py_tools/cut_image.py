...
def save_loc_png(img, center_QSO, c_psf_list):
    ...
    cax=ax.imshow(np.log10(img+0.5),origin='lower', vmax=2.1)
    QSO_box_size = 50
    QSO_reg = pix_region(center_QSO, radius= QSO_box_size)
    QSO_mask = QSO_reg.to_mask(mode='center')
    ax.text(center_QSO[0]-QSO_box_size, center_QSO[1]+QSO_box_size, 'QSO', fontsize=20)
    ax.add_patch(QSO_mask.bbox.as_patch(facecolor='none', edgecolor='red'))
    PSF_box_size = 20
    for i in range(len(c_psf_list)):
        ...
        ax.text(c_psf_list[i][0]-PSF_box_size, c_psf_list[i][1]+PSF_box_size, 'PSF{0}'.format(i), fontsize=15)
