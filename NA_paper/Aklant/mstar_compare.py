...
plt.scatter(MBs, Mstar, c=zs,s=280,marker="o",zorder=100, vmin=1.2, vmax=1.8, edgecolors='k')
cl=plt.colorbar()          #cl take the inforamtion from the lastest plt
plt.clim(1.2,1.8)
cl.set_label('Source redshift',rotation=270,size=20)

x_aklant = np.linspace(7,9.2)
y_aklant = np.linspace(10.5,11.8)
plt.plot(x_aklant-0.25-0.15,y_aklant-0.15, x_aklant-0.15, y_aklant-0.15, x_aklant+0.25-0.15,y_aklant-0.15,color = 'r')

x_local = np.linspace(6, 9.9)
y_local = np.linspace(9,12.5)
plt.plot(x_local-0.4,y_local, x_local, y_local, x_local+0.4,y_local,color = 'black')


plt.xlabel("$log(M_{BH}/M_{\odot})$",fontsize=35)
plt.ylabel("$log(M_*/M_{\odot})$",fontsize=35)
plt.minorticks_on()
