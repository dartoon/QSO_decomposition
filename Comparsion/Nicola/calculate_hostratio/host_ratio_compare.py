...
siml_mags = np.loadtxt('lagn_lhost_Vband.dat')
host_f = 10.**(-0.4*(siml_mags[:,0]))
agn_f = 10.**(-0.4*(siml_mags[:,1]))

plt.figure(figsize=(8,6))
high0, x0, _ = plt.hist(host_f/(agn_f+host_f)*100.,normed=True, histtype=u'step',
...
x0_m = np.median(host_f/(agn_f+host_f)*100.)
