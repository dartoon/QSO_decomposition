...
This is only for include the MBII data in the plot. Run MBII data first
"""
import numpy as np
...
#    plt.errorbar(np.log10(1+Knud[:,0]),Knud[:,2]-(m_ml*Knud[:,4]+b_ml),yerr=0,fmt='o',color='blue',markersize=9) 
    plt.errorbar(np.ones(len(bhmass_selected))*np.log10(1+1.5),bhmass_selected-(m_ml*mstar_selected+b_ml),
                 yerr=0,linestyle=' ',marker='o',ms=10,mec='k', alpha=0.5) #!!! Run "my_selection_plots_MBII.py" first
    plt.errorbar(np.log10(1+1.5), np.median(bhmass_selected-(m_ml*mstar_selected+b_ml)),
                 yerr=0,linestyle=' ',marker='o',ms=12,color='red',zorder = 100) #!!! Run "my_selection_plots_MBII.py" first
