...
plt.errorbar(kor_e[:,1],kor_e[:,3], xerr=kor_e[:,2] ,yerr=kor_e[:,4],fmt='.',color='red',markersize=15, label='Kormendy2013 elliptical')
...
plt.errorbar(kocb_e[:,1],kocb_e[:,3], xerr=kocb_e[:,2] ,
             yerr=kocb_e[:,4],fmt='.',color='blue',markersize=15, label='Kormendy2013 classical bulge')
...
#plt.text(9.3, 6.0, "log$(M_{BH}/10^{7}M_{\odot})$=%s+%slog$(M_*/10^{10}M_{\odot})$"%(round(b_ml0+m_ml0*10-7,2),round(m_ml0,2)),color='black',fontsize=25)
#plt.text(9.3, 6.25, "log$(M_{BH}/10^{7}M_{\odot})$=%s+%slog$(M_*/10^{10}M_{\odot})$"%(round(b_ml1+m_ml1*10-7,2),round(m_ml1,2)),color='red',fontsize=25)
