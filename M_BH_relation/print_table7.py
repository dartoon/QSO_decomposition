...
for i in range(len(b11)):
    print b11name[i] + " & {0:.3f} & {1:.2f}({5:.2f})$\pm${2:.1f} & {3:.2f}$\pm${4:.1f} \\\\".format(
            b11[i,0],  b11[i,1], 0.2, b11[i,2], 0.4, b11[i,3])

#%%
ssname = ['158', '170', '271', '273', '305', '333', '339', '348', '379', '413', '417', '465', '516', '540', '597', '712']  #ID 250 and 375 is deleted
for i in range(len(ss)):
    print  ssname[i] + " & {0:.3f} & {1:.2f}({5:.2f})$\pm${2:.1f} & {3:.2f}$\pm${4:.1f} \\\\".format(
            ss[i,0], ss[i,1], 0.2, ss[i,2], 0.4, np.log10(10**(ss[i,1])*ss[i,3]))
