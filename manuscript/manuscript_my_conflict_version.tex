...
Correlations between the mass of a supermassive black hole (SMBH) and the properties of its host galaxy (e.g. stellar mass \smass, luminosity \lhost) suggests an evolutionary connection. A powerful test of a co-evolution scenario is to measure the relations \mbh-\lhost\ and \mbh-\smass~at high redshift and compare with local estimates. For this purpose, we acquired multi-band {\it Hubble Space Telescope} imaging with WFC3 of 32 X-ray-selected broad-line (type-1) AGN at $1.2<z<1.7$ in deep survey fields. By applying state-of-the-art tools to decompose the \hst\ emission, we measured the host galaxy luminosity and stellar mass along with other properties through 2D model fitting. The black hole mass (\mbh) is determined using the broad \halpha~line, detected in the near-infrared with Subaru/FMOS, which potentially minimizes systematic effects using other indicators. We find that the  {\it observed} ratio of \mbh/\smass~is larger at $z\sim1.5$ by $0.31\pm0.1$ dex as compared to that in the local universe, while the scatter is as low as in the local universe. However, when accounting for selection effects (estimated using two independent methods), we find that no-evolution is also consistent with the data at the 95\% CL, both using our sample by itself and in combination with literature data. The relationship between \mbh\ and host galaxy total luminosity paints a similar picture. Therefore, given the uncertainties, our results are in agreement with a scenario where SMBHs and the total host stellar mass and luminosity proceed in lockstep or the growth of the former is somewhat slightly anticipated. We then use a matched sample of inactive galaxies to estimate the bulge-to-total mass fraction and show that the bulge \mbh/\smass~relation is significantly offset from the local one, even accounting for selection effects. Thus, our observations are consistent with a scenario in which stellar mass and luminosity is transferred from the angular momentum supported to the pressure supported component of the galaxy through secular processes or minor mergers at a faster rate than that of mass accretion onto the SMBH.
...
\subsection{Rest-frame colors (THIS SECTION MAY NEED TO BE EXPANDED ON)}

For 18/32 AGNs, we have multi-band host magnitudes which enables us to select the best stellar population template that could be applicable to determine rest-frame luminosities and stellar masses for the overall sample. We find that a 1~Gyr stellar population with solar metallicity for a Chabrier IMF~\citep{Bruzual2003} could well match the colors of our sample based on WFC3 and ACS band magnitudes.
 
\subsection{\mbh-\lhost\ relation}\label{sec:ml}

Adopting the 1~Gyr stellar population, we perform a K-correction to derive the rest-frame R-band magnitude of our sample. With the WFC3 filter already close to the rest-frame R-band, we expect the $M_R$ uncertainty introduced by this K-correction to be within 0.05~mag. We derive the rest-frame R band luminosity from $\log L_R/L_{R, \odot} = 0.4\times(M_{R, \odot}-M_R)$, where $M_{R, \odot}=4.61$~\citep{Blanton07}. $L_R$ ranges between $\log L_R \in [9.5, 11.5]$ ($L_{\odot,R}$) with individual values listed in Table~\ref{tab:result_sersic}. 
...
\noindent which enables comparison between our high-$z$ sample and the local relation. The distribution of our data appears to be in good agreement with the local relation and the other AGN samples at lower redshift. Therefore, the observational data indicate that the relation between black hole mass and the host luminosity are similar at different period of the universe. In Appendix~\ref{sec:ml-ev}, we explore how the high-$z$  sample would evolve in this plane solely with luminosity evolution of the host galaxy as done in past studies \citep[e.g., ][]{Ding2017b}. 
...
{\bf [Include a full paragraph on the scatter]} As shown in these panels, there is some scatter similar in magnitude to the local relation. 
...
The local sample of inactive galaxies used in this analysis is mainly comprised of bulge-dominated galaxies and the adopted \smass\ are equivalent to their bulge masses. That is, we are comparing the \mbh-\smass$_{\rm ,total}$ relations in the distant universe to the \mbh-\smass$_{\rm ,bulge}$ relations locally. Considering that a significant stellar component of the high-$z$ AGNs have a disk component (i.e., AGN hosts with fitted \sersic\ index close to 1),  \smass$_{\rm , bulge}$ will be smaller than \smass$_{\rm ,total}$.
...
stellar mass. We can now compare this \smass$_{\rm, bulge}$ to that
of the black hole \mbh. As expected, we find a stronger evolutionary
trend with $\gamma = 1.63\pm0.27$, which corresponds to an offset of
$0.65\pm0.11$ dex in \mbh/\smass$_{\rm, bulge}$ with respect to the
local relation. Since selection effects are independent of B/T in
...
confirms with higher fidelity the concusion by \citet{Bennert11}
...
In this work, we use the start-of-the-art techniques to derive the host flux from the AGN image. The fidelity of the inferred apparent magnitudes of the host galaxies are high. Nevertheless, we adopt one stellar population for the full sample to derive the rest-frame R-band luminosity and stellar mass.
We note that adopting a stellar population model separately for each AGN host could be more robust in principle. However, considering that the host magnitude in the HST/ACS band is faint (host-to-total flux ratio $< 30\%$), the fidelity of the SED fitting is actually lost. Perhaps the best way to derive the stellar population is to combine our measurements with ground-based photometry. This is currently beyond the scope of this analysis.
...
We highlight that the uncertainty in the scaling relations is dominated by the single epoch black hole mass estimates. That is to say, the intrinsic scatter is not necessarily worse at high-$z$ than the low-$z$.

\subsection{Systematic errors related to the choice of local anchor}
\label{sec:local_sys}

To compare our high-$z$ sample to the local relation, we adopt the local measurements by \citet{Ben++10, Bennert++2011} and  \citet{H+R04}. In the literature, other analyses of the local sample are available which could also be considered as the local anchor. We note that the different local samples have inconsistencies which could affect our understand of co-evolution. To estimate this effect, we compare our local \mbh-\smass\ sample to that in \citet{Kormendy13} (hereafter K13). We find that the \mbh-\smass\ measurements in K13 have a global $\sim+0.3$~dex offset relative to our local sample. Note that a $+0.3$~dex difference could totally erase the $\Delta\log$\mbh\ for the high redshift sample as described in last section.

However, we realize that the \mbh-\sigstar\ relations by K13 also have a $\sim$0.3~dex offset to those in \citet{Woo2010}. Note that it is the local \mbh-\sigstar\ relation by \citet{Woo2010} that is used to determine the virial coefficient for our active BHs. Thus, if adopting the K13 normalization, the \mbh\ of all the AGN sample should be increased by $\sim$0.3~dex. As a result, adopting different local measurement would only globally affect the level of the \mbh\ to local and high-$z$ sample at the same time, with the result of  $\Delta\log$\mbh\ unchanged. We thus conclude that, if adopt K13 sample, the inference of the high redshift offset of $\Delta\log$\mbh\ would remain at the same level. However, this quick comparison of the local sample highlights the requirement of a solid local baseline for the future studies (Bennert et al., in prep).
...
We studied the evolution of the correlations between the supermassive black hole and their host galaxies using new measurements of 32 X-ray selected AGNs at $1.2<z<1.7$. Near-infrared spectroscopic observations with Subaru/FMOS of the \halpha\ emission line are available that provide reliable BH masses. To obtain the properties of the AGN host galaxies, we performed an image decomposition using the state-of-the-art techniques on \hst/WFC3 IR data to obtain high-resolution imaging of the AGN and its host. This required us to construct a collection of PSF-stars across all the fields to build to a library of PSFs. Using the latest image modeling tool \lenstronomy, we decomposed the AGN image in 2-D plane taking each PSF in the library. We obtained the host properties (i.e., host flux, effective radius, \sersic\ index) using a weighted arithmetic mean based on the inference from the eight top-ranked PSFs. With additional \hst/ACS image in the optical, we identified the appropriate stellar population and derived the rest-frame R-band Luminosity (\lhost) and stellar mass (\smass) of the host with the corresponding mass-to-light ratio.

We then determine the mass relations at high-$z$ and establish any signs of evolution by combining our high-$z$ measurements with local and intermediate redshift samples from the literature~\citep{Park15, Bennert11, SS13, Cisternas2011}. With consideration of selection biases, we find the average ratio between the mass of a supermassive black hole and either their total host luminosity or stellar mass at $z\sim1.5$ does not exhibit a significant deviation, as compared to local values, for which we can consider to be evidence of the growth of one preceding the other. Thus a co-evolutionary scenario is favorable. However, there is marginal room for the black hole to overshoot the growth of its host, although less than a factor of two, since there may be selection effects that accounted for as precisely as needed. 

Regarding the black hole mass relative to the bulge mass that is primarily the quantities that are responsible for the local relations, we find that our high-$z$ AGN sample clearly has elevated black hole masses thus indicative of strong evolution. Comparing to their \mbh, we find evolution with the ratio evolving as $(1+z)^{1.63\pm0.27}$. This analysis is based on an inference of the B/T ratio from the measured Sersic indices. A proper measure of the bulge mass at high-$z$ may need to wait for the next generation ground-based 20-30m class telescopes. Even so, this conclusion seems solid that the BHs 8-10~Gyrs ago reside in bulges which are less massive/luminous than today.

Of further interest, the scatter in the \mbh-\smass\ ratios at high-$z$ is similar to the local scatter. This was unexpected since the measurements of the masses of both the black holes and their hosts should have more uncertainty that local estimates. If true, this may indicate that cosmic averaging of initially unrelated states are not driving the relation between SMBHs and their host galaxies thus there is likely a connection between the two yet to be fully understood, contenders include AGN feedback or links to common gas reservoirs.
...
Column~12: Bulge stellar mass, based on Section~\ref{sec:bh_bulge} and Figure~\ref{fig:BT-n_relation}.
}
\end{deluxetable*}
