...
\altaffiltext{4}{Dipartimento di Matematica e Fisica, UniversitÃ Roma Tre, via della Vasca Navale 84, I-00146, Roma, Italy}
...
\section{Expected selection effect bias} \label{sec:sf_framework}
Our AGN sample is selected based on the value of \mbh, Eddington ratio, and \halpha\ broad emission line, with low nuclear-to-host ratios It has been known that sample selection effects could influence the observed black hole-host relations and its evolution with redshift ~\citep{Schulze2011, Tre++07, Bennert++2011, Park15}. In theory, the simulations~\citep{DeG++15} also find the scaling relations by the selected sample has steeper feature than by the random ones. To highlight this influence, we study the expected bias of the \mbh\ given our target selection in Section~\ref{sec:target_selection}.

We use the Bayesian framework introduced by \citet{Schulze2011} to estimate the expected bias. Given our particular selection window (\mbh,  $L_{\rm bol}$ and Eddington ratio $\lambda$) and the local sample, the framework considers the fact that the intrinsic scatter in the BH-bulge relation has trend to decline regime of the stellar mass function, result in an average stellar masses lower than the relation. Moreover, the other relevant sources of bias are considered in this framework, for example the active fraction among SMBHs varies by the black hole mass function, in the sense that BHs with higher mass are less likely to be active than lower mass ones. 

Adopting our specific selection limits into the framework (i.e., $\log($\mbh$)~\in[7.5, 8.56]$, $\log(L_{\rm bol}) \in [45.0, 46.2] $ and  $\log(\lambda) \in [-2.0, 0.5]$), we infer an expected bias of $+0.21$~dex in the observed $\Delta\log($\mbh$)$ for samples at $z\sim1.5$.

We would correct our inference by taking this select bias into account in the result sections.

%Interestingly, this bias could account for the offset that obtained in the \mbh-\smass\ correlations in Section~\ref{sec:mm}. We use green open circles to point out the shifting of the expected population given these biases, as shown in Figure~\ref{fig:MM-vz}.
...
The host-total flux ratio of the sample are varies from 10\% to 90\% (concentrated between 20\% to 50\%). The effective radius and \sersic\ index are randomly distributed within the prior, rather than biased on upper/lower side. The inferred \Reff\ of our sample are between $\sim$1-7 kpc (peaked at $\sim$2.2kpc). 15/32 systems have \sersic\ index $n<2$, indicating they are composed of disk+bulge components.  Five systems have high \sersic\ index ($n>4.5$); we use $n\in[1,4]$ as prior to re-fit these systems and find that the changes on the inference of their host luminosity are very limited ($<0.03$ dex).

We also comparing the distribution of the \sersic\ values to the inactive galaxies in the Appendix~\ref{sec:comp_inactive} and find that our high-$z$ AGN host have very similar distribution.

We calculate the rest-frame R band magnitude ($M_R$) of our sample based on their filter magnitude. 18/32 systems have multi-band host magnitude which enables us to select the best stellar population that could fit the overall sample. We find that the 1~Gyrs stellar population with solar metallicity by Chabrier initial mass function (IMF)~\citep{Bruzual2003} could well match the color between the WFC3 and ACS band magnitude of these samples.
...
We visually see that in Figure~\ref{fig:MM-vz}, panel (a), most of our measurements locate within the scattering range of the local sample. We doubt whether the positivity of $\gamma$ stems from the three outliers discussed in last section. We thus exclude these outliers and re-run the statistical analysis to see if offset still exists. As a result, we obtain that $\gamma  = 0.65 \pm 0.27$, indicating the positive evolution trend become weaker but still exists.
...
It is worth note that the local sample employed are bulge-dominated galaxies and the adopted \smass\ are equivalent to their bulge masses. That is, we are comparing the \mbh-\smass$_{,total}$ relations at the distant universe to the \mbh-\smass$_{,bulge}$ relations at local. Considering that a major part of the distant systems has indicated the disk component (i.e., the systems with fitted \sersic\ index close to 1), the \smass$_{, bluge}$ of them are supposed to be even less. Given that our 32 AGNs present the similar distribution to the inactive ones in CANDLES (see Appendix~\ref{sec:comp_inactive}), we expect the bulge-total ratio could be follow the similar ratio.

{\color{red}
[Xuheng will add one more section here, about the bulge \smass\ inference and the corresponding $\gamma$, after comparing to the CANDLES data.]
}
...
\label{select_eff}
In Section~\ref{sec:sf_framework}, we adopt the Bayesian framework by \citet{Schulze2011} and find that there is an expected $+0.21$~dex bias in the inferred $\Delta \log$\mbh. Interestingly, this bias could account for the observed offset as obtained in the \mbh-\smass\ correlations in last Section. %~\ref{sec:mm}.
To make it clear, we use green open circles to point out the shifting of the expected population given these biases in Figure~\ref{fig:MM-vz}. We also show, in Figure~\ref{fig:offset_hist}, the histogram the $\Delta \log$\mbh\ by the high-$z$ sample and compare to the local ones. Clearly, the correction by considering this selection effect could almost mitigate the offset.
The result of the correction shows that the high-$z$ sample could have consistent \mbh-\smass$,_{\rm host}$ to the local sample.
...
To compare our high-$z$ sample to the local relation, we adopt the local measurements by \citet{Ben++10, Bennert++2011, H+R04}. In the literature, other local sample have been measured which could also be considered as the local anchor. We note that different local samples have inconsistency which could change the result of the co-evolutions. To estimate this effect, we compare our local \mbh-\smass\ sample to the local ones by \citet{Kormendy13} (hereafter K13). We find that the \mbh-\smass\ measurements in K13 have a global $\sim+0.3$~dex offset to our local ones, and if adopted would totally erase the offset of $\Delta\log$\mbh\ for high redshift sample.

However, we also realize that the \mbh-\sigstar\ relations by K13 also have a $\sim$0.3~dex offset to the ones by \citet{Woo2010}. Note that it is the local \mbh-\sigstar\ relation by \citet{Woo2010} that is used to determine the virial coefficient for our active BHs. Thus, if adopting the K13's sample, the \mbh\ of all the AGN sample should be increased by $\sim$0.3~dex. As a result, the different local measurement would only affect the level of the \mbh\ globally to both local and high-$z$ sample, with the offset unchanged. 

We thus conclude that, if adopt K13 sample, the inference of the high redshift $\Delta\log$\mbh\ would remain at the same level. However, this quick comparison of the local sample highlights the need of using the solid local baseline for the future studies (Bennert et al., in prep).
