...
\textcolor{blue}{
In this study, we aim to make progress by making use of a large sample with high-quality data both for the measurement of both the host properties and the \mbh, extending to the highest redshifts where evolutionary effects should be strongest.}%~\citep{DeG++15}.
...
We aim to determine whether AGNs have begun to couple to their host galaxy at $1.2<z<1.7$, an epoch when supermassive BH close to its peak accretion history~\citep{Aird2015}.  
We design to choose our sample that meets the following criterion to overcome the limitations of previous studies.
\begin{enumerate}
\item A sample of broad-line (type-1) AGNs that fall well below the knee of the black how mass function to minimize the bias which could raised by the selection effect.
\item Sufficient Eddington ratios to assure the AGN contains accreting black hole.
\item Balmer lines are available to estimate their \mbh, which avoids potential systematic uncertainties in the UV-based estimators \citep{Greene2005}.
\item X-ray selected sample results in lower nuclear-to-host ratios facilitating the galaxy mass measurements.
\item Images of the host galaxy observed by adopted \hst/WFC3 above the 4000 \angstrom\ break, given their redshift range.
\item The large proportion of the sample has multi-band \hst\ imaging, providing extra color information to achieve reliable K-correction and stellar mass inference. 
\end{enumerate}

In redshift range $1.2<z<1.7$, we will measure 32 host galaxies properties using \hst\ imaging data, and estimate their \mbh\ based on the robust \halpha\ and \hbeta, using the multi-object spectrograph Subaru/FMOS. Given the high-quality and sample size of our data, we are capable of testing whether the growth of BH predates that of the host by a factor of at least $1.7$ \citep[i.e. $\sim0.23$ dex,][]{Schulze2014}. 

\subsection{Target selection}
We focus on broad-line AGNs as provided by the X-ray coverage of COSMOS~\citep{Civano2016}, (E)-CDFS-S~\citep{Lehmer2005, Xue2011}, and SXDS~\citep{Ueda2008} fields, with X-ray flux limited in 0.5-8 keV.
The Near IR spectra of the AGNs in these fields are available from the survey of Subaru's Fiber Multi-Object Spectrograph~\citep[FMOS, ][]{Kimura2010, Matsuoka2013, Nobuta2012}, covering the wavelength range 0.9-1.8~$\mu m$ which provides the favorable \halpha\ and \hbeta\ lines out to $z\sim1.7$ to estimates the \mbh. More recently, \citet{Schulze2018} present near-IR spectroscopy of a large sample of 243 X-ray AGN in these field. We referred to that paper for the details of continuum fitting and emission line modeling. 
\textcolor{red}{John comments to be included:
[These were first identified through optical spectroscopy in most cases with zCOSMOS bright of faint samples (see Merloni et al. 2007) and then followed up with NIR spectroscopy.]}


Using the \mbh\ estimator introduced in Section~\ref{mbh}, we estimate the value of \mbh\ and select the targets meets range $7.5 \lesssim {\rm log}$~\mbh$\lesssim8.5$.
The bolometric luminosities presented in \citet{Schulze2018} enable us to calculate their Eddington ratio $\lambda = L_{\rm bol}/L_{\rm Edd}$ and select the sample which meets our selection window, as shown in Figure~\ref{fig:selection}. Moreover, we have higher preference to select the target which has rest-frame UV images as provided by \citet{Scoville2007}.

We list all the AGN systems analyzed in this work in Table~\ref{tab:objlist}.
...
Having defined the recipes, we estimate \mbh\ by adopting the emission line properties measured by \citet{Schulze2018}. 14/32 systems have both \halpha\ and \hbeta\ emission line information; we adopt the averaged value for log(\mbh). \textcolor{blue}{3/32 systems, including CDFS-1, CDFS-229 and CDFS-724, are not included in \citet{Schulze2018}; their \halpha\ emission lines information is provided by \citet{Suh2015}}.
...
We include in our analysis published samples at intermediate redshift range to understand the redshift evolution of these correlations. We select samples that were analyzed by members of our team, in order to ensure uniform measurements. The intermediate redshift AGNs that we consider include, for the \mbh-\lhost\ relation, 52 objects published by \citet{Park15} at $0.36<z<0.57$ and 27 objects published by \citet{Bennert11, SS13} at $0.5<z<1.9$. Moreover, \citet{Bennert11, SS13} also derive the \smass\ for the 27 objects which enable us to compare to the \mbh-\smass\ relation. We recalibrate the \mbh\ of the intermediate sample using the self-consistent recipes introduced in Section~\ref{mbh}.
...
We visually see that in Figure~\ref{fig:MM-vz}, panel (a), most of our measurements locate within the scattering range of the local sample. We doubt whether the positivity of $\gamma$ stems from the four outliers (i.e., the top-four systems in Figure~\ref{fig:MM}). We thus exclude these systems and re-run the statistical analysis to see if offset still exists among the rest of our sample. As a result, we obtain that $\gamma  = 0.55 \pm 0.29$, indicating the positive evolution trend become weaker but still exists, as shown in Figure~\ref{fig:MM_restsample}.
...
\textcolor{red}{To be updated with Andres's result.}
...
\subsection{Systematic by different local anchor}
...
To compare with the local relation, we adopt the local measurements by \citet{Ben++10, Bennert++2011, H+R04}. 
%This adopting enable a direct comparison with the previous work \citep{Park15, SS13}, which adopted the same local sample. 
We realize that, in the literature, there are other local measurements have been studied which could also be considered as the local anchor. For example, we present our local \mbh-\smass\ sample and the ones by \citet{Kormendy13} (hereafter K13) together in Figure~\ref{fig:local_MM_comp}. To test whether different local sample changes the conclusion of the co-evolution, we use different combinations of the these sample and compare their difference by fitting them as linear relation, i.e. Equation~\ref{eq:MMlocal}. It turns out that the local anchor inferred by HR04+B11 has a lower intercept value $\alpha$ compared to the one by K13+B11 by 0.3~dex. As a consequence, adopting the local by K13+B11 would raise the local relation, hence mitigate the offset of $\Delta\log\mathcal M_{\rm BH}$ and $\gamma$ by a factor of 0.3~dex. Given the values in Table~\ref{fig:MM_restsample}, this mitigating wouldn't change our inference of $\gamma$ from positive to negative. However, this quick comparison of the local sample highlights the need of the solid local baseline for the future studies (Bennert et al., in prep).
