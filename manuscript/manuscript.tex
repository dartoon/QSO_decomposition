...
{\bf The value for the all the \mbh-\lhost\ comparison sample are listed in \citet{Ding2017b} (in Table~1 and Table~2). The \mbh-\smass\ comparison sample are collected and summarized in the Appendix~\ref{sec:comp_sample_value}, Table~\ref{tab:comp_sample}.}
...
{\bf Having obtained the \hst\ image, we remove the background light which could come from both the sky and the detector. In this step, we adopt the {\sc photutils} by Python and model the global background light in 2D based on the \texttt{SExtractor} algorithm. Then, we subtract the globe sky background light to obtain the clear image. To test the fidelity of this subtraction, we investigate the surface brightness at the empty regions and make sure it is neither positive nor negative. Finally, we extract postage stamp of the AGN image and PSF image to carry out the modeling process, see Section~\ref{sec:psf_library} and Section~\ref{sec:analysis}.} 
...
To compare our high-$z$ sample to the local relation, we adopt the local measurements by \citet{Ben++10, Bennert++2011} and \citet{H+R04}. In the {\bf literature~\citep{Kormendy13, Bentz2018}}, other analyses of the local sample are available, which could also be considered as the local anchor. We note that the different local samples have inconsistencies, which could affect our measurement of the co-evolution. {\bf In this paper, we aim to make the differential measurement as function of redshift. With this goal in mind, the consistency between the local and distant sample is crucial in terms of measurement procedure. This consistency is related to how measurement systematics and calibrations propagate between the galaxy dynamical masses v.s. galaxy stellar masses, bulge v.s. total and \mbh\ by single epoch v.s. reverberation v.s. dynamical.
...
For the local by \citet{Bentz2018}, with different goal in mind, they decompose their host using more components than bulge$+$disk (e.g., Bar(s), Barlens, Ring(s)). This is beyond the comparison medium, i.e., bulge v.s. disk, of ours. Moreover, with a more manageable model, our inference gets ride of the overfitting issue. In spite of this, when adopting the \citet{Bentz2018} as local anchor, we obtain $\gamma = 2.12\pm 0.32$ using the 32 AGNs, indicating still a positive evolution.
 }
...
Second, we adopt the common stellar templates to derive the host luminosity and stellar mass for overall sample, which introduces some uncertainty in the inference of the host properties. Even so, the uncertainty in the scaling relations is still dominated by the single epoch black hole mass estimates (see Section~\ref{sec:sysm_err} for detail). Third, for several AGN systems, their inferred host effective radius hit the lower limit, and thus the inference of their \Reff\ should be considered as upper limit. However, the inference of their host luminosities and stellar masses are still reliable, given our re-fitting test using different lower limit boundary and the low scatter of the host flux inference (see Section~\ref{sec:result-hosts}, second paragraph). Last, in the literature, there are several different sample could be choose as local anchor to the high-$z$ sample, which could changes the inference of the evolution value. In this work, we adopted the most similar local sample to those at high-$z$. We discussed the different anchors in detail and test to infer the evolution different anchors; all the inference show the positive evolution  (see Section~\ref{sec:local_sys}).}
...
\begin{table}
\centering
\caption{The summary for the comparison sample}\label{tab:comp_sample}
\resizebox{18cm}{!}{
