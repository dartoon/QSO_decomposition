...
A powerful way to understand the origin of these correlations is to study them as a function of redshift, determining how and when they emerged and evolved over cosmic time~\citep[e.g.,][]{TMB04,Sal++06,Woo++06, Jah++09,SS13}. During the past decade, there has been much progress on this front at $z<1$ using type 1 AGNs. For example, the work by \citet{Park15, Tre++07, Pen++06qsob} demonstrated that \mbh\ at fixed mass resided in less luminous galaxies than today. Similarly, the works by \citet{Bennert11, Woo++08} find the positive evolution of \mbh\ in the observation of the \mbh-\smass and \mbh-\sigstar relations. \textcolor{blue}{In addition, based on spectroscopic information, \citet{Merloni2010} decomposed the entire spectral energy distributions (SED) into a nuclear AGN and host-galaxy components, finding a positive evolution of black holes to the galaxies in their correlation.
This offset can be explained by a scenario in which supermassive BHs were built up first with galaxy then growing around their deep potential wells.
It should be kept in mind, however, that the evolution is detected at low significance given the considerable error bars and limited size of the sample. For example, some studies report results consistent with no evolution given their larger error bars \citet{SS13, Sun2015, Cisternas2011}. In addition, once considering the selection effect \citet{Mechtley2012, Schulze2014}, the observed apparent offset at high redshift is fully consistent with no intrinsic offset.}
...
\item Images of the host galaxy observed by adopted \hst/WFC3 above the 4000 \angstrom\ break and below the broad \halpha\ line (6563 \angstrom) given their redshift range.
...
We also test to use $n\in[1,4]$ as prior, and find that the changes on the inference of the host properties are very limited.
}}.
...
To compare our high-$z$ sample the local relation, we adopt the local measurements by \citet{Ben++10, Bennert++2011, H+R04} as the anchor at today. In the literature, there are other local measurements have been studied which could also be considered as the local anchor. We realize that the inconsistency in the different local samples shift our local anchor and thus changes the result of the co-evolutions. 

For example, we compare our local \mbh-\smass\ sample to the local quiescent galaxies in \citet{Kormendy13} (hereafter K13). We find that K13's local \mbh-\smass\ relation would raise the anchor by $\sim$0.3~dex. This could mitigate our offset value $\gamma$ by 0.3~dex. However, we also note that the \mbh-\sigstar\ relations in K13 is also higher than the one by our local sample. Note that the \mbh-\sigstar\ relations by the quiescent galaxy are usually used to determine the virial coefficient for AGNs. That is, adopting the K13 would boost the value of our \mbh\ by $\sim$0.3~dex as well. As a result, our inference of the fitted offset $\gamma$ still remains at the same level.
