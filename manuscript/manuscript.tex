...
{\bf The values for the all the \mbh-\lhost\ comparison sample are listed in \citet{Ding2017b} (in Table~1 and Table~2). The \mbh-\smass\ comparison sample are collected and summarized in the Appendix~\ref{sec:comp_sample_value}, Table~\ref{tab:comp_sample}.}
...
For the local sample by \citet{Bentz2018}, with different goal in mind, they decompose their host using more components than bulge$+$disk (e.g., Bar(s), Barlens, Ring(s)). This is beyond the comparison medium, i.e., bulge v.s. disk, of ours. Moreover, with a more manageable model, our inference gets ride of the overfitting issue. In spite of this, when adopting the \citet{Bentz2018} as local anchor, we obtain $\gamma = 2.12\pm 0.32$ using the 32 AGNs, indicating still a positive evolution.
 }
