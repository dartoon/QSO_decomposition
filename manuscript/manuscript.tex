...
\tablecolumns{7}
\tablewidth{0pt}
\tablecaption{The process of weighting for COSMOS-CID1174 } 
\tablehead{ 
\colhead{PSF rank} &
\colhead{total $\chi ^2$} & 
\colhead{weights $w_i$} &
...
\tablecomments{
The detailed information of the weights for the inference of CID1174. The inflation value $\alpha$ calculated by Eq.~\ref{eq:alpha} is 16.671.
}
\end{deluxetable*}
...
\colhead{magnitude}&
\colhead{reduced $\chi ^2$}& \colhead{host flux ratio}& \colhead{magnitude}\\
\colhead{}& 
\colhead{}& \colhead{}& 
\colhead{($\arcsec$)}& \colhead{}& 
\colhead{(AB system)}& \colhead{}& 
\colhead{}& \colhead{(AB system)} \\
\colhead{(1)}& 
\colhead{(2)}& \colhead{(3)}& 
\colhead{(4)}& \colhead{(5)}& 
\colhead{(6)}& \colhead{(7)}& 
...
\colhead{Target ID}&
  ...
  \cline{2-4}  \cline{5-7} \\
\colhead{}& 
\colhead{FWHM}& \colhead{$\lambda \rm L _{H\alpha}$}& \colhead{\mbh}&
\colhead{FWHM(5100)}& \colhead{$\lambda \rm L _{\lambda5100}$}& \colhead{\mbh}\\
\colhead{}& 
\colhead{(\kms)}& \colhead{($10^{42}{\rm erg~s^{-1}}$)}& 
\colhead{(M$_{\odot}$)}& \colhead{(\kms)}& 
\colhead{($10^{44}{\rm erg~s^{-1}}$)}&\colhead{(M$_{\odot}$)}\\
\colhead{(1)}& 
\colhead{(2)}& \colhead{(3)}& 
\colhead{(4)}& \colhead{(5)}& 
\colhead{(6)}&\colhead{(7)}
