...
\altaffiltext{3}{{\color{blue} Kavli Institute for the Physics and Mathematics of the Universe (WPI), The University of Tokyo, Kashiwa, Chiba 277-8583, Japan}}
...
{\color{blue}
We find that the observed ratio of \mbh/\smass is larger at $z\sim1.7$ than the one in the local universe. However, when accounting for the selection effect (using two independent methods), we find that the high redshift points can be consistent with the local relation, assuming the scatter is unchanged or larger. The relationship between \mbh\ and host galaxy total luminosity paints a similar picture: the ratio is larger than the local universe, but given the uncertainties and selection effects one cannot conclude that the offset is significant. Therefore, given the uncertainties, our results are consistent with a scenario where SMBHs and the total host stellar mass and luminosity proceed in lockstep or the growth of the former is somewhat anticipated. Considering that the local relationships general apply to bulges, and our hosts have a significant amount of stellar mass in a disk, observations are consistent with a scenario in which stellar mass and luminosity is transferred from the angular momentum supported to the pressure supported component of the galaxy by secular processes or minor mergers at a faster rate than that of mass accretion onto the SMBH.
}
...
We construct a sample of broad-line (FWHM$>2000$ km s$^{-1}$; type-1) AGNs that meet the following criteria to overcome limitations of previous studies:
...
Our AGN sample is selected based on the value of \mbh, Eddington ratio, and \halpha\ broad emission line, with low nuclear-to-host ratios. It is well known that sample selection effects must be taken into account in order to  interpret the observed black hole-host relations and measure its evolution with redshift avoiding bias ~\citep{Schulze2011, Tre++07, Bennert++2011, Park15}. The main source of bias is due to the fact that active samples are necessarily selected based on properties that correlate with black hole mass estimators (like AGN luminosity and line strength and width) and thus one tends to favor overly massive black holes in presence of intrinsic scatter and observational errors.

Correcting for observational biases requires a well-characterized selection function, such as the one we have for our sample, and a model of the black hole mass function, and the evolution of the correlations between \mbh, and other properties.

In order to get a sense for the amount of bias that we can anticipate given our selection function, we use the Bayesian framework introduced by \citet{Schulze2011} to estimate the expected bias. In this framework, under the assumption of no evolution of the correlations between \mbh\ and host-galaxy properties, one can compute the expected bias for a given sample, prior to observations. The key ingredient of this model are the local \mbh\ host-galaxy property correlations, the black hole mass function and Eddington ratio distribution at the redshift of observation. The latter two quantities are estimated from the type-1 AGN distributions corrected to represent the parent population of all SMBH hosts, including the inactive ones. 

Adopting our specific selection limits into the framework (i.e., $\log($\mbh$)~\in[7.5, 8.56]$, $\log(L_{\rm bol}) \in [45.0, 46.2] $ and  $\log(\lambda) \in [-2.0, 0.5]$), we infer an expected bias of $+0.21$~dex in the observed $\Delta\log($\mbh$)$ for samples at $z\sim1.5$, assuming the baseline choices for the inputs to the model. {\bf TT: we should say something about the uncertainty here for example the following.} This bias correction should be considered an approximate estimate, with an uncertainty that depends on the uncertainty of the inputs. Furthermore, if the scaling relations actually evolve, one needs to introduce a model for the evolution in order to infer the underlying bias-corrected trends. We will revisit these issues in Section~\ref{select_eff}.
...
{\color{red} To avoid any systematic bias between samples in the literature, we aim to adopt a class of self-consistent estimators for our sample. We first compare the estimators implemented in \citet{Schulze2018} to the self-consistent ones in \citet{Ding2017b}, and find that their \hbeta(FWHM(5100)) estimators are very consistent (\mbh\ r.m.s. $<0.03$~dex). However, there is a $\sim~0.2$ dex inconsistency in their \halpha\ estimators. Thus, we adopt all the AGNs in \citet{Schulze2018} which have both \halpha\ and \hbeta\ lines (35 AGNs in total) and carry out a cross-calibration to select the \halpha\ estimator which has better consistency. The result show that the \halpha\ estimators in \citet{Schulze2018} demonstrate the better agreement, which are adopted by this work:
%Thus, we adopt the scheme given in \citet{Schulze2018} that is based on the recalibration of these relations using luminous quasars out to high redshift \citep{Jun2015} :

\begin{figure}
\centering
{
\includegraphics[height=0.55\textwidth]{fig/cross-calibrate.pdf}
}
\caption{\label{fig:cross-calibrate} 
Cross-calibration using the AGNs in \citet{Schulze2018} for \halpha\ and \hbeta\ estimators. The \citet{Schulze2018} demonstrate better agreement (\mbh\ mean offset $\sim~0.09$~dex) than \citet{Ding2017b} (\mbh\ mean offset $\sim~0.27$~dex). {\bf Xuheng: The Jun+2015's recipe has a 0.4 dex mismatch in this comparison.}}
\end{figure} 
}
...
We include in our analysis published samples at intermediate redshifts to understand the evolution of these correlations. We select samples that were analyzed by members of our team, in order to ensure uniform measurements. The intermediate redshift AGNs that we include are 52 objects published by \citet{Park15} applicable for the \mbh-\lhost\ relation at $0.36<z<0.57$ and 27 objects published by \citet{Bennert11} and \citet{SS13} at $0.5<z<1.9$. In addition, we adopt a sample of 32 \mbh-\smass\ measurements at $0.3<z<0.9$ by \citet{Cisternas2011} to compare with our sample. 

{\color{red} For all the comparison AGN samples, we recalibrate the \mbh\ using the self-consistent recipes introduced in Section~\ref{mbh}. For \mbh\ estimated using \Mgii\ and  \hbeta$(\sigma_{\rm H\beta})$ lines, we adopt the recipes by \citet{Ding2017b}. }
...
For each of the 32 new AGNs, we obtained six separate exposures with $\sim399s$ (i.e. total exposure time $\sim2394s$). The six exposures for each dither image were combined with the {\sc astrodrizzle} software package, with an output pixel scale of 0\farcs{0642} by setting \texttt{pixfrac} parameter as 0.8 using the \texttt{gaussian} kernel\footnote{\label{note1}For CID255, 3/6 of the dither WFC3 images are corrupted. We achieve to analyze this sample using the same approach, taking the 3 available frames.}. 
To accurately estimate the background light which could come from both the sky and the detector, we adopt the {\sc photutils} tool. We list the details of the observation in Table~\ref{tab:objlist}.

Multi-band information provides the SED at a more precise level. 21/32 objects in our sample have rest-frame UV images by \citet{Scoville2007}. Images were taken through the wide ACS/F814W filter at four dither pointing with $507s$ exposures. The final image is drizzled to 0\farcs{03} pixel scale. Given the multi-band image for these systems, we are able to infer their host color and assess the contribution of both the young and old stellar population which insures an accurate inference of rest-frame R-band luminosity by K-correction, stellar mass and star formation rate (SFR) \citep{Gallazzi2009} . 
...
To avoid any unphysical inferences, we set the upper and lower limit for the key parameters as effective radius \Reff\ $\in[0\farcs{}1,1\farcs{0}]$, and \sersic\ index $n\in[1,7]$.
...
In Figure~\ref{fig:AGN_decomp}, we demonstrate the optimized results inferred by \lenstronomy, including the AGN images, best-fit models, image subtract point source (i.e. host) and the residuals, taking COSMOS-CID1174 as an example. The weights adopted in the analysis are listed in Table~\ref{tab:weight_CID1174}. 
...
Illustration of the corner relations between the inferred host \sersic\ parameters.}
\end{figure*} 

We apply this approach to the overall AGN systems to infer the overall host galaxy information. To quickly overview, we plot the relations of the properties between effective radius (\Reff), \sersic\ index and host to total flux ratio in Figure~\ref{fig:flux_r_n_corner} and leave the detail descriptions in the next Section. We realize that the inference of the host properties is weighted by eight top-ranked PSFs. The smaller volume of top-ranked  PSFs would possibly underestimate the uncertainties of the inference. To understand how the choice of the number of top-ranked PSF affects our inference, we compare to the inference by using five and ten top-ranked PSFs. As shown in Figure~\ref{fig:hist_compare}, the results are very consistent, meaning that the host inference has been solid when the amount of selected PSF exceeds five. 
...
Comparison of the histogram of the inferred host flux ratio, based on the different the top-ranked PSFs number.}
\end{figure*} 
...
In previous section, we carry out the AGN-host decomposition and derive the host galaxy \sersic\ properties of the overall sample, as shown in Figure~\ref{fig:hist_compare}. The optimized resulting images for each sample are presented in the figures in Appendix~\ref{sec:restsample}. Based on the third column of these resulting figures (i.e. the ``data-Point source'' stamps), we find that all the systems have shown the evidence for existence of the host galaxies. We also find that merger feature of the host galaxies are not strong, showing that our sample possibly not be dominated by the major merger.

The host-total flux ratio of the sample are varies from 10\% to 90\% (concentrated between 20\% to 50\%). The effective radius and \sersic\ index are randomly distributed within the prior, rather than concentrating on upper/lower side. The inferred \Reff\ of our sample are between $\sim$1-7~kpc (peaked at $\sim$2.2~kpc). 15/32 systems have \sersic\ index $n<2$, indicating they are composed of disk+bulge components.  Five systems have high \sersic\ index ($n>4.5$); we use $n\in[1,4]$ as prior to re-fit these systems and find that the changes on the inference of their host luminosity are very limited ($<0.03$ dex).
...
18/32 systems have multi-band host magnitude which enables us to select the best stellar population that could fit the overall sample. We find that the 1~Gyrs stellar population with solar metallicity by Chabrier initial mass function (IMF)~\citep{Bruzual2003} could well match the color between the WFC3 and ACS band magnitude of these samples. Given this template, we are capable of inferring the rest-frame R band luminosity (\lhost) and stellar mass (\smass) of our sample based on their filter magnitude, in the following sections.
 
\subsection{\mbh-\lhost\ relation}\label{sec:ml}
Adopting the 1~Gyrs stellar population, we perform the K-correction to derive the rest-frame R band magnitude of our sample. Given that the WFC3 filter we selected is very close to the rest-frame R band, we expect the $M_R$ uncertainty introduced by this K-correction is within 0.05~mag. We derive the rest-frame R band luminosity by $\log L_R/L_{R, \odot} = 0.4 (M_{R, \odot}-M_R)$, where $M_{R, \odot}=4.61$~\citep{Blanton07}. The inferred $L_R$ are ranged between $\log L_R \in [9.5, 11.5]$ ($L_{\odot,R}$), as summarized in Table~\ref{tab:result_sersic}. 
...
We note three outlier measurements deviated from the distribution of overall sample, which are SXDS-X763, CDFS-1, CDFS-724. We expect that they could have abnormal host inference. Indeed, for SXDS-X763, the inferred host \Reff\ has large scatter ($\sim 75\%$) with the lowest host-total flux ratio in the overall sample ($<10\%$). For CDFS-1 and CDFS-724, their host are \Reff$<0\farcs{}15$, smaller than the PSF FWHM. Moreover, the broad emission line of \mbh\ are from \citet{Suh2015} which could have systemic to \citet{Schulze2018}. Thus, the measurements of these three systems have less fidelity. 
...
Taking the stellar population, we estimate the stellar mass content of each host galaxy according to the mass-to-light ratio of the stellar population we adopted. The uncertainty level associated with the stellar mass is expected to be of order 0.1~dex, at fixed stellar initial mass function (changing the initial mass function would change all our stellar masses systematically). The inferred $M_*$ are ranged between $\log M_* \in [9.7, 11.3]$ ($M_{\odot}$), listed in Table~\ref{tab:result_sersic}.

In Figure~\ref{fig:MM}, we plot the our \mbh-\smass\ measurements and compare to the local and intermediate sample. We visually see that the relations for the three samples are close. Similar as Equation~\ref{eq:MLlocal}, we consider the local \mbh-\smass\ relations as linear relation as:
\begin{eqnarray}
\label{eq:MMlocal}
...
\end{eqnarray} 
where $\Delta \log \mathcal M_{\rm BH} = \log \big( \frac{\mathcal M_{\rm  BH}}{10^{7}M_{\odot}}) -\alpha-\beta\log(\frac{M_*}{10^{10}M_{\odot}})$. We obtain $\gamma  = 0.84 \pm 0.26$, i.e. tentative evidence for evolution, see Figure~\ref{fig:MM-vz}, panel (a). We also show the ratio of \mbh-\smass\ of the three samples at different redshift in Figure~\ref{fig:MM-vz}, panel (b). 
...
Left: illustration of the offset in  $\log($\mbh$)$ (VS. \smass) as a function of redshift. The orange band is the intrinsic scatter of local linear relation. The red line and gray band are the best-fit and $1-\sigma$ offset fitting by Equation~\ref{eq:offset} using our 32 high-$z$ AGNs.
...
As shown in Figure~\ref{fig:MM-vz}, panel (a), most of our measurements locate within the scattering range of the local sample. We doubt whether the positivity of $\gamma$ stems from the three outliers discussed in last section. We thus exclude these outliers and re-perform the statistical analysis to see if the offset still exists. As a result, we obtain that $\gamma  = 0.65 \pm 0.27$, indicating the positive evolution trend become weaker but still exists.
...
It is worth note that the local sample employed are bulge-dominated galaxies and the adopted \smass\ are equivalent to their bulge masses. That is, we are comparing the \mbh-\smass$_{,total}$ relations at the distant universe to the \mbh-\smass$_{\rm ,bulge}$ relations at local. Considering that a major part of the distant systems has indicated the disk component (i.e., the systems with fitted \sersic\ index close to 1), the \smass$_{, bluge}$ of them are supposed to be even less. 
{\color{blue}
%1. B/T can be inferred by n
%2. The sample we use and how to build up the relation.
%3. take the relation and infer the bulge. Finally infer the result
%4. We note the scatter could be larger.
Given that the inferred morphology of our 32 AGN hosts presents the similarities to the CANDELS inactive ones (see Appendix~\ref{sec:comp_inactive}), we expect their B/T follows the similar distribution which can be roughly estimated by the single-\sersic\ index \citep{Bruce2014}. With this estimation, we could roughly infer the bulge stellar mass of our AGN host.

To this end, we first study the relations between B/T and single-\sersic\ index for CANDELS inactive galaxies at the similar mass level, i.e \smass$\in [9.0, 11.5]$, using B/T measured by \citet{Dimauro2018}. Then, we calculate the local averaged  B/T value at different \sersic\ index value to build up their specific relation, as shown in Figure~\ref{fig:BT-n_relation}. Taking this relation, we estimate the B/T of our AGN sample using the inferred \sersic\ index as proxy and infer their \smass$_{, bulge}$. We take this \smass$_{, bulge}$ to study their co-evolution to the \mbh. As expected, we find a stronger evolution trend with $\gamma = 1.71\pm0.27$. Of course, this is a very rough estimate --- the systematic for the estimated \smass$_{, bulge}$ must be larger than the \smass$_{, total}$.
...
The relations between stellar mass bulge-total-ratio and single-\sersic\ index by CANDELS inactive galaxies with \smass$\in [9.0, 11.5]$. The red curve is the inferred specific relation taking the average B/T ratio at different \sersic\ index, used as proxy to infer the B/T for our AGN sample.
}
\end{figure} 
}
...
\centering
    \caption{The inference of $\gamma$ using \mbh-\smass\ by different sample combination.}\label{table:diff_sample_gam}
     ...
     &\\
     \hline\hline
32 AGNs & 0.84$\pm$0.26\\    
29 AGNs (excluded outliers) & 0.65$\pm$0.27\\    
32 AGNs + intermediate &  0.62$\pm$0.20\\
32 AGNs bulge only & 1.71$\pm$0.27\\
     \hline
     \end{tabular}}
...
In order to evaluate the ``true" underlying evolution and its uncertainty, corrected for selection effects, taking into account the actual observations, we adopt an independent method based on the approach introduced by \citet{Tre++07} and developed by \citet{Park15, Ding2017b}. The method parametrizes the evolution of the correlations between \mbh\ and host galaxy properties as an offset from the local one $\gamma$ and an intrinsic scatter \sint, which can be a free parameter or tied to the local relation. It then imposes a selection function in \mbh\ (assuming that the distribution of Eddington radio is narrow and therefore has a second order effect) and calculates the intrinsic parameters of the model given the observations. In practice, we start from the local black hole mass function and the evolution model, generate mock samples using a Monte Carlo approach, apply the selection function and compare with the data to generate the likelihood. 

To illustrate the importance of the intrinsic scatter in the selection effects, we adopt both uniform (flat) prior and lognormal prior for \sint.

Combining the 32 AGNs together with the intermediate redshift sample, we present the inferred  $\gamma$ and \sint\ in the two-dimensional planes in Figure~\ref{fig:select_effect}. The plots show that the inferred evolution is uncertain and depends crucially on the intrinsic scatter. Assuming the lognormal prior \sint, to mimic the assumption of the method discussed in Section~\ref{sec:sf_framework}, one sees that the best estimate of $\gamma$ is positive, but the 95\% confidence intervals extend to zero. Thus one cannot conclude that evolution is significantly detected in our data. 
...
\centering
    \caption{The summary for the different inference of $\gamma$.}\label{table:gamma_sf}
     ...
     &&&\\
     \hline\hline
32 AGNs + intermediate & No & 0.62$\pm$0.20 & 0.66$\pm$0.18 \\
32 AGNs + intermediate& Yes & 0.80$\pm$0.30 & 0.60 $\pm$ 0.30 \\
32 AGNs & No & 0.84$\pm$0.26  & 1.07$\pm$0.23\\
...
\label{sec:local_sys}
To compare our high-$z$ sample to the local relation, we adopt the local measurements by \citet{Ben++10, Bennert++2011, H+R04}. In the literature, some other local sample have been measured which could also be considered as the local anchor. We note that different local samples have inconsistency which could affect our understand of the co-evolutions. To estimate this effect, we compare our local \mbh-\smass\ sample to the local ones by \citet{Kormendy13} (hereafter K13). We find that the \mbh-\smass\ measurements in K13 have a global $\sim+0.3$~dex offset to our local sample. Note that a $+0.3$~dex difference could totally erase the $\Delta\log$\mbh\ for the high redshift sample as described in last section.

However, we realize that the \mbh-\sigstar\ relations by K13 also have a $\sim$0.3~dex offset to the ones by \citet{Woo2010}. Note that it is the local \mbh-\sigstar\ relation by \citet{Woo2010} that is used to determine the virial coefficient for our active BHs. Thus, if adopting the K13's sample, the \mbh\ of all the AGN sample should be increased by $\sim$0.3~dex. As a result, adopting different local measurement would only globally affect the level of the \mbh\ to local and high-$z$ sample at the same time, with the result of  $\Delta\log$\mbh\ unchanged. 

We thus conclude that, if adopt K13 sample, the inference of the high redshift offset of $\Delta\log$\mbh\ would remain at the same level. However, this quick comparison of the local sample highlights the requirement of a solid local baseline for the future studies (Bennert et al., in prep).
...
We used the published near-infrared spectroscopic observations \halpha\ emission lines to estimate the reliable values of BH mass. To obtain the properties of the AGN host galaxies, we performed the AGN image decomposition using the state-of-the-art techniques. We adopted the \hst/WFC3 infrared channel to observe the high-resolution AGN imaging data and collected the PSF-stars in across all the fields to build to a library of PSF for the fitting. Using the latest image modeling tool \lenstronomy, we decomposed the AGN image in 2-D plane taking each PSF in the library. We obtained the host \sersic\ property (i.e., host flux, effective radius, \sersic\ index) using a weighted arithmetic mean based on the inference by the eight top-ranked PSFs. Incorporating with the inference by \hst/ACS image, we selected the stellar population, hence derived the rest-frame R band Luminosity (\lhost) and stellar mass (\smass) of the host.

Combining our high-$z$ measurements with the intermediate redshift samples from the literature~\citep{Park15, Bennert11, SS13, Cisternas2011}, we compared the scaling relations to the ones from the local with main results summarized as follows:
\begin{enumerate}
\item The \mbh-\smass\ relations at higher redshift are inconsistent with the local ones, i.e. Figure~\ref{fig:MM}, \ref{fig:MM-vz}. Considering the correction for passive evolution of galaxy luminosity, the \mbh-\lhost\ relations at higher redshift are inconsistent with the local ones,  i.e. Figure~\ref{fig:ML-vz} - (a).
\item The offsets of by the evolution are well described by the form $\Delta\log\mathcal M_{\rm BH}= \gamma \log (1 + z)$. We obtain $\gamma = 0.84\pm0.26$ for \mbh-\smass\ relation.

\item However, when considering the selection effect (using two independent methods), we find that the high redshift measurements can be consistent with the local relations, i.e. Section~\ref{select_eff}.

\item Our inference indicates the evidence of disk+bulge component (i.e. the ones with \sersic\ $n<2$), while the local comparing samples are bulge dominated. We take the inferred  \sersic\ $n$ as proxy to estimate the bulge-total-ratio stellar mass. Comparing to their \mbh, we find a strong evolution with $\gamma = 1.71\pm0.27$. Thus, the conclusion seems solid that the BHs 8-10~Gyrs ago reside in bulges which are less massive/luminous than today.

\item We also compare the properties of our active galaxy sample to the ones from inactive galaxy in the CANDELS imaging survey at the same redshift range, finding no significant discrepancy among them.
\end{enumerate}
...
{\color{blue} More acknowledges to be updated. }
