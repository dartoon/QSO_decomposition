...
ploc[:,1]=4.61  - (0.4*(4.61+0.46-4.83)+ploc[:,1]-0.4*dm*pass_dmag(ploc[:,0]))/0.4  #Change to Mr,
...
x=loc[:,0]
y=loc[:,1]
yerr=(loc[:,2]**2+(0.2/0.4)**2)**0.5  # 0.2 is the uncertainty level for the L_R, M_R is 0.2/0.4
...
xl = np.linspace(-26, -17.5, 100)
m, b, sint =np.percentile(samples, 50,axis=0)
plt.plot(xl, m*xl+b, color="k", linewidth=4.0,zorder=10)
...
for i in range(100):
    ...
    rec[i,0],rec[i,1]=m,b
    plt.plot(xl, m*xl+b, color="lightgray", alpha=0.1,linewidth=7.)
