...
for i=0L,n_elements(input_gam)-1L do marginal_Pgam[i] = total(exp(0.5D*(grid_2lnP[i,*] - max_2lnP)))  ; = sum(p/pmax)
	...
	XYOUTs,best_gam_marginal+0.02,(max(marginal_Pgam)+min(marginal_Pgam))/2D*0.9D,textoidl('\gamma=')+string(best_gam_marginal,format='(D3.1)')+textoidl('\pm')+string((max(gam_1sig) - min(gam_1sig))/2D,format='(D3.1)'),$
...
for j=0L,n_elements(input_sig)-1L do marginal_Psig[j] = total(exp(0.5D*(grid_2lnP[*,j] - max_2lnP)))  ; = sum(p/pmax)
	...
	XYOUTs,(max(marginal_Psig)+min(marginal_Psig))/2D*0.98D,best_sig_marginal-0.035,textoidl('\sigma_{int}=')+string(best_sig_marginal,format='(D4.2)')+textoidl('\pm')+string((max(sig_1sig) - min(sig_1sig))/2D,format='(D4.2)'),$
