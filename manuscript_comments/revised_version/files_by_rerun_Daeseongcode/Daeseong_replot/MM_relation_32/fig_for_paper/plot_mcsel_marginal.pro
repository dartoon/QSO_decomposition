...
for j=0L,n_elements(input_sig)-1L do marginal_Psig[j] = total(exp(0.5D*(grid_2lnP[*,j] - max_2lnP)))  ; = sum(p/pmax)
	...
	XYOUTs,(max(marginal_Psig)+min(marginal_Psig))/2D*0.99D,best_sig_marginal-0.035,textoidl('\sigma_{int}=')+string(best_sig_marginal,format='(D4.2)')+textoidl('\pm')+string((max(sig_1sig) - min(sig_1sig))/2D,format='(D4.2)'),$
