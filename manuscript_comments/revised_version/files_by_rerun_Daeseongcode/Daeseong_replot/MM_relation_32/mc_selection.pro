...
common baseline, local
	...
	;log M = alpha + beta*(log M - 11) + gamma*log(1+z)
	;py = local.alpha + local.beta * px + dellogM
	;[log M - alpha - gamma*log(1+z)] / beta = (log M - 11)
...
common baseline, local
;(Xuheng requests) combination of samples of Bennert+11 and Haring & Rix 04 local inactive M-Mbul relation (logM = alpha + beta log Mbul/10^11)
local = { $
	alpha   :8.31D, $
	beta    :1.02D, $
	int_scat:0.36D}
...
;---------------------------------------------------------
;-read data
READCOL,'../../data_MBH-Mhost_NewSampleOnly.txt',name,z,logMBH,elogMBH,logL,elogL,flag,format='A,D,D,D,D,D,L',count=Ndata,comment='#'
...
;sig_int = 0.27D                                                                                                                                        
;esig_int = 0.10D                                                                                                                                       
;4) Bennert+11 and HR04 local inactive glaaxy M-M relation sig_int = 0.36+/-0.07                                                                                        
;-rederived by us for M-M relation with linmix_err
sig_int = 0.36D                                                                                                                                        
esig_int = 0.07D                                                                                                                                       
