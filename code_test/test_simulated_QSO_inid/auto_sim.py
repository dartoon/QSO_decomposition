...
for i in range(len(ini_n_sq)):
    for j in range(len(host_ratio_sq)):
        ...
        for k in range(1):
            print 'Sm_{0}_In_{1}_Rato_{2}.txt'.format(int(sim_n),int(ini_n),int(host_ratio*10))
#            runfile('{0}/simulated_test.py'.format(dir_path), wdir=dir_path)
            
#for i in range(len(ini_n_sq)):
#    for j in range(len(host_ratio_sq)):
#        sim_n,ini_n, host_ratio = 3., ini_n_sq[i], host_ratio_sq[j]
#        for k in range(1):
#            print 'Sm_{0}_In_{1}_Rato_{2}.txt'.format(int(sim_n),int(ini_n),int(host_ratio*10))
#            runfile('{0}/simulated_test.py'.format(dir_path), wdir=dir_path)
#            
#for i in range(len(ini_n_sq)):
#    for j in range(len(host_ratio_sq)):
#        sim_n,ini_n, host_ratio = 5., ini_n_sq[i], host_ratio_sq[j]
#        for k in range(1):
#            print 'Sm_{0}_In_{1}_Rato_{2}.txt'.format(int(sim_n),int(ini_n),int(host_ratio*10))
#            runfile('{0}/simulated_test.py'.format(dir_path), wdir=dir_path)
#            
