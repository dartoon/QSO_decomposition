...
deltaPix = 0.0642  #  pixel size in arcsec (area per pixel = deltaPix**2)
psf_type = 'PIXEL'  # 'gaussian', 'pixel', 'NONE'
PSF0 = pyfits.getdata('LID1273_PSF.fits')[30:-30,30:-30]
PSF0 /= PSF0.sum()
...
data_class = SimAPI.data_configure(numPix, deltaPix, 1, 0)
...
#plt.matshow(np.log10(image_ps_sim), origin='lower')
#plt.colorbar()
#plt.show()
...
#plt.matshow(np.log10(image_host_sim), origin='lower')
#plt.colorbar()
#plt.show()
print 'total_flux:', image_noisy.sum()
print 'host_total_flux: ',image_host_sim.sum(),'\npoint source flux: ',image_ps_sim.sum()
host_ratio_cal = image_host_sim.sum()/(image_host_sim.sum()+image_ps_sim.sum())
#print 'host/total ratio:', host_ratio_cal
#==============================================================================
# To do the fitting:
#==============================================================================
fixed_source, kwargs_source_init,kwargs_source_sigma,kwargs_lower_source, kwargs_upper_source= [], [], [], [], []
#fixed_source.append({'n_sersic': 1.,'R_sersic': .3})
fixed_source.append({})
kwargs_source_init.append({'R_sersic': .3, 'n_sersic': ini_n, 'e1': 0., 'e2': 0., 'center_x': 0., 'center_y': 0.})
...
PSF1 = pyfits.getdata('LID1538_PSF.fits')[30:-30,30:-30]
PSF1 /= PSF1.sum()
...
#                                   gridspace = 'log',if_annuli=True,norm_pix=4.0)
#plt.show()
...
source_result, ps_result, image_ps, image_host, data_C_D=fit_qso(QSO_im=image_noisy, psf_ave=PSF1, source_params=source_params,pix_sz = 'drz06',
                                                                 ...
                                                                 exp_time = exp_time,tag=tag, no_MCMC=True)
...
#print 'Sim_n:', sim_n, 'ini_n:', ini_n, 'input_host_ratio:', round(host_ratio_cal*100,1), '%\nfit_result:\n', result
...
if if_file == []:
    ...
    f.read()
f.write("\n#========================================")    
f.write("\n#Sim_n: "+repr(sim_n)+ ' ini_n: '+repr(ini_n)+ ' input_host_ratio: '+repr(round(host_ratio_cal*100,1))+'%')
