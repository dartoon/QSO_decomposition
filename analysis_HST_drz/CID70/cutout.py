...
#c_psf_list = grab_pos(filename,reg_ty = 'org')
#print c_psf_list

fitsFile = pyfits.open('idnl14010_drz.fits')
img = fitsFile[1].data  #- (-0.003)  # check the back grounp
center_QSO = np.array([869,547])
...
count=0
psf_list = np.array([[571,227],[792,519],[702,524],[866,725],[334,258],[376,433]])
for i in range(len(psf_list)):
    PSF = cut_center_bright(image=img, center=psf_list[i], radius=30)
...
#    pyfits.PrimaryHDU(PSF).writeto('PSF{0}.fits'.format(count),overwrite=True)
#    count += 1
save_loc_png(img,center_QSO,psf_list, ID=ID)
